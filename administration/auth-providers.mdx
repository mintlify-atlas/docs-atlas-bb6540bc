...
TMT uses two complementary authentication systems: **Auth0** for identity management and login flows, and **Firebase Authentication** for back-end service access. Route guards built on top of these systems control which pages users can access based on their authentication state and role permissions.

## Authentication providers

<Tabs>
  <Tab title="Auth0">
    Auth0 is the primary identity provider for TMT. It handles user login, session management, and token issuance.

    **Package**: `@auth0/auth0-spa-js`  
    **Context file**: `src/guards/auth0/Auth0Context.js`

    ### How it works

    At app startup, `Auth0Context` initializes an `Auth0Client` instance using the environment variables `VITE_AUTH0_CLIENT_ID` and `VITE_AUTH0_DOMAIN`. It then calls `checkSession()` to restore any existing session silently.
    ...
    Login is triggered via a popup:

    ```javascript
    await auth0Client.loginWithPopup();
    ```

    After a successful login, the user object is retrieved with `auth0Client.getUser()` and stored in React context. The context exposes `isAuthenticated`, `user`, `signin`, `logout`, and `resetPassword`.
